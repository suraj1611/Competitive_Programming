...
int main()
{
	...
	char f,s,t;
	if(s.count(A>B)&&s.count(A>C)||s.count(B<A)&&s.count(C<A)||s.count(A>B)&&s.count(C<A)||s.count(B<A)&&s.count(A>C))
		f='A';


		f='B';
	else if(s.count(A>B)&&s.count(A>C)||s.count(B<A)&&s.count(C<A))
		f='C';
